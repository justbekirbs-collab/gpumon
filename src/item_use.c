...
static void ItemUseOnFieldCB_DexNav(u8 taskId);
static void ItemUseOnFieldCB_RaidPass(u8 taskId);
...
void FieldUseFunc_DexNav(u8 taskId)
{
    sItemUseOnFieldCB = ItemUseOnFieldCB_DexNav;
    SetUpItemUseOnFieldCallback(taskId);
}
...
void FieldUseFunc_RaidPass(u8 taskId)
{
    sItemUseOnFieldCB = ItemUseOnFieldCB_RaidPass;
    SetUpItemUseOnFieldCallback(taskId);
}
...
{
    u16 species = (Random() % (NUM_SPECIES - 1)) + 1;
    u8 level = GetHighestLevelInPlayerParty() + 5;
    if (level < 1) level = 1;
    if (level > 100) level = 100;
    ...
    // Boost HP for Raid Boss
    {
        u16 maxHP = GetMonData(&gEnemyParty[0], MON_DATA_MAX_HP, NULL);
        maxHP *= 3; // 3x HP for raid boss
...
static void ItemUseOnFieldCB_PcAccess(u8 taskId)
{
...
{
    sItemUseOnFieldCB = ItemUseOnFieldCB_PcAccess;
    SetUpItemUseOnFieldCallback(taskId);
}
...
static void Task_BattleUse_Overclocker_DelayAndPrint(u8 taskId);
void BattleUseFunc_Overclocker(u8 taskId)
{
    u8 battler = gBattlerInMenuId;
    if (gBattleStruct->overclockedGpus & (1 << battler))
    {
        DisplayItemMessageInBag(taskId, FONT_NORMAL, gText_WontHaveEffect, Task_ReturnToBagFromContextMenu);
    }
    else
    {
        gBattleStruct->overclockedGpus |= (1 << battler);
        gTasks[taskId].data[8] = 0;
        gTasks[taskId].func = Task_BattleUse_Overclocker_DelayAndPrint;
    }
}
...
static void Task_BattleUse_Overclocker_DelayAndPrint(u8 taskId)
{
    ...
    {
        PlaySE(SE_USE_ITEM);
        DisplayItemMessageInBag(taskId, FONT_NORMAL, gText_Overclocked, Task_BattleUse_StatBooster_WaitButton_ReturnToBattle);
    }
}
