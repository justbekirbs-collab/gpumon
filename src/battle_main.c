...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum)
{
     ...
     && !(gBattleTypeFlags & (BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_TRAINER_TOWER)))
    {
        ...
        if (gTrainers[trainerNum].trainerClass == TRAINER_CLASS_CHAMPION || gTrainers[trainerNum].trainerClass == TRAINER_CLASS_ELITE_FOUR)
            scaledLevel = highestLevel + 2;
        else if (gTrainers[trainerNum].trainerClass == TRAINER_CLASS_LEADER)
            scaledLevel = highestLevel + 1;
        else
            scaledLevel = highestLevel - 2 + (Random() % 4);

        if (scaledLevel < 1) scaledLevel = 1;
        if (scaledLevel > 100) scaledLevel = 100;
...
u8 GetWhoStrikesFirst(u8 battler1, u8 battler2, bool8 ignoreChosenMoves)
{
    ...
    if (gBattleStruct->overclockedGpus & gBitTable[battler1])
        speedBattler1 = (speedBattler1 * 150) / 100;
    if (gBattleStruct->overclockedGpus & gBitTable[battler2])
