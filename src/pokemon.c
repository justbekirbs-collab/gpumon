...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (gBattleStruct->overclockedGpus & gBitTable[battlerIdAtk])
    {
        attack = (attack * 150) / 100;
        spAttack = (spAttack * 150) / 100;
        gBattleMovePower = (gBattleMovePower * 150) / 100;
    }
    if (gBattleStruct->overclockedGpus & gBitTable[battlerIdDef])
    {
...
void Cheat_GiveTopGPUs(void)
{
    u16 move1 = MOVE_PSYCHIC;
    u16 move2 = MOVE_SURF;
    u16 move3 = MOVE_CUT;
    u16 move4 = MOVE_STRENGTH;
    u32 shinyPersonality = 0;

    // === BLACKWELL (MEWTWO) - Ultimate AI ===
    ...
    // === TEACH CUT AND SURF TO ALL PARTY MEMBERS ===
    {
        u16 moveCut = MOVE_CUT;
        u16 moveSurf = MOVE_SURF;
        int i;
        for (i = 0; i < 6; i++) {
            GiveMoveToMon(&gPlayerParty[i], moveCut);
            GiveMoveToMon(&gPlayerParty[i], moveSurf);
        }
    }

    // === JUSTBEKIR RIG (Master Ball) x999 in BAG + x999 in PC ===
    AddBagItem(ITEM_MASTER_BALL, 99);
    AddPCItem(ITEM_MASTER_BALL, 999);
    ...
    AddBagItem(ITEM_OVERCLOCKER, 1);

    // === HM CUT + HM SURF in bag ===
